...
class VistaTkinter:
    ...
    def realizar_pedido(ventana_raiz):
        ...
        opcion_categoria = tk.OptionMenu(
            ventana_hija.ventana, indice_categoria, *option_list)
        ...
        articulos = Controlador.obtener_articulos_por_categoria(
            int(indice_categoria.get()))
        ...
        def callback(*args):
            ...
            categoria_seleccionada = (
                Controlador.obtener_nombre_categoria(valor)).upper()
            articulos_categoria = Controlador.obtener_articulos_por_categoria(
                valor)
            articulos_caja.limpiar()
            [articulos_caja.insertar(art) for art in articulos_categoria]
            etiqueta_categoria.etiqueta.configure(
                text="{}".format(categoria_seleccionada))
    ...
    def realizar_cobro(ventana_raiz):
        ...
        caja_txt_numero_orden = CajaTexto(
            ventana=ventana_hija.ventana, variable=numero_orden)
        ...
        def vista_introducir_cedula():
            ventana_hija = VTopLevel(
                ventana_raiz.ventana, "Introducir numero de cedula")
            ventana_hija.ventana.geometry("384x162+500+200")
            ventana_hija.ventana.configure(background='white')
            ...
            caja_txt_cedula_identidad = CajaTexto(
                ventana=ventana_hija.ventana, variable=cedula_identidad)
            caja_txt_cedula_identidad.invocar_pack()
            boton_siguiente = Boton(ventana=ventana_hija.ventana,
                                    nombre="Siguiente", color="white",
                                    evento=lambda: validar_cliente(
                                        cedula_identidad.get())
                                    or ventana_hija.salir())
        ...
        def seleccionar_metodo_pago():
            ventana_hija = VTopLevel(
                ventana_raiz.ventana, 'Seleccion de metodo de pago')
            ventana_hija.ventana.geometry("300x180+500+200")
            ventana_hija.ventana.configure(background="white")
            ...
            boton_opcion_efectivo.invocar_pack()
            boton_opcion_efectivo.boton.configure(width=15)
            VistaTkinter.agregar_espaciado(ventana_hija)
            boton_opcion_tarjeta.invocar_pack()
            boton_opcion_tarjeta.boton.configure(width=15)
        ...
        def realizar_cobro():
            ...
            VistaTkinter.info('Cobro realizado con exito', str(comprobante))
    ...
    def obtener_informe(ventana_raiz, anio, mes, semana, dia):
        ...
        if anio and anio.isdigit():
            ...
            if mes and mes.isdigit():
                ...
                mensaje = Controlador.filtrar_comprobantes(
                    Controlador.definicion_filtro_comprobante_anual(anio))
        else:
            VistaTkinter.error(
                'Entrada Invalida', 'Debe introducir el anio, obligatoriamente para realizar el filtro')
        if mensaje:
    ...
    def menu_principal():
        ...
        ventana_raiz = VTopLevel(
            vista_principal.ventana, 'Sistema de pedidos para farmacias')
...
class CajaTexto():
    ...
    def __init__(self, ventana, variable):
        self.CajaTexto = Entry(
            ventana, textvariable=variable, background="light gray")
